...
import { Plus, ArrowUpRight, TriangleAlert, Mail, ShieldCheck, MapPin, Trash2 } from 'lucide-react';
...
/** Wash reps (field pickup/delivery agents) — list, add, assign area, suspend, remove. */
export function WashRepsTab() {
  ...
  async function deleteRep() {
    if (!deleting) return;
    try {
      // Soft-remove: set inactive so they drop out of assignments; history is kept.
      await api.patch(`/reps/${deleting.id}`, { status: 'inactive' });
      setDeleting(null);
      load();
    } catch {
      setDeleting(null);
      ...
      key: 'menu', header: '', render: (r) => (
        <RowMenu
          items={[
            ...
            { label: 'Delete Rep', icon: <Trash2 size={14} />, danger: true, onClick: () => setDeleting(r) },
          ]}
        />
      ),
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmModal
        ...
        icon={<Trash2 size={20} />}
        danger
        title={`Delete ${deleting?.user?.fullName ?? 'Rep'}?`}
        body="This removes the rep from the platform — they're set inactive and drop out of all assignments. Their order history and earnings are preserved. You can re-add them later."
